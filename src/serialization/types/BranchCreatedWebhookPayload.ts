// This file was auto-generated by Fern from our API Definition.

import type * as Webflow from "../../api/index";
import * as core from "../../core";
import type * as serializers from "../index";

export const BranchCreatedWebhookPayload: core.serialization.ObjectSchema<
    serializers.BranchCreatedWebhookPayload.Raw,
    Webflow.BranchCreatedWebhookPayload
> = core.serialization.object({
    siteId: core.serialization.string().optional(),
    branchId: core.serialization.string().optional(),
    pageId: core.serialization.string().optional(),
    parentPageId: core.serialization.string().optional(),
    pageName: core.serialization.string().optional(),
    createdBy: core.serialization.string().optional(),
    createdOn: core.serialization.date().optional(),
});

export declare namespace BranchCreatedWebhookPayload {
    export interface Raw {
        siteId?: string | null;
        branchId?: string | null;
        pageId?: string | null;
        parentPageId?: string | null;
        pageName?: string | null;
        createdBy?: string | null;
        createdOn?: string | null;
    }
}
