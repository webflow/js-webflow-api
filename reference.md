...
<Warning title="Enterprise & site token only">This endpoint requires an Enterprise workspace and a [site token](/data/reference/site-token) with the `workspace_activity:read` scope. Please create a site token from your site settings in the Webflow dashboard to use this endpoint.</Warning>
